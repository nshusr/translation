...
| 名称       | 类型                                                          | 必填 | 描述                                                                                                                                                                                                      |
| ---------- | ------------------------------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `where`    | `UserWhereInput`                                              | 否   | 将_所有_模型字段包装在一个类型中，以便可以通过任何属性过滤列表。                                                                                                                      |
...
#### 返回 `Post` 记录按关联 `User` 记录的 `name` 排序
...
##### 排序 `User` 按多个字段 - `email` _和_ `role`

以下示例按两个字段排序用户：- 首先按 `email`，然后按 `role`
...
排序参数的顺序很重要 - 以下查询按 `role`，然后按 `email` 排序。不是结果的差异：
...
##### 排序 `User` 按 `email`，选择 `name` 和 `email`
...
##### Get all `User` records where _所有_ posts are published and a least one related `Post` mentions `Prisma`
##### 获取_所有_已发布的 `用户` 帖子，并且至少有一个相关的 `Post` 提到了 `Prisma`
...
返回所有 **all**("every") _关联_记录匹配过滤条件的记录
...
##### 获取_全部_已发布帖子的所有 `User`，并且至少有一个相关的 `Post` 提到了 `Prisma`
...
The following query returns all users where the `sanctuaries` array contains _所有_ the values in the given array:
